...
/*****************************************************************************/
{
  ...
  sub_ = node_->create_subscription<nav_msgs::msg::OccupancyGrid>(
    "map_slam", rclcpp::QoS(1), mapCallback);
}
...
/*****************************************************************************/
{
  ...
  if (name != "") {
    ...
    int rc = system(("ros2 run nav2_map_server map_saver_cli -f " + name  + " --ros-args -p map_subscribe_transient_local:=true -r map:=map_slam" + set_namespace).c_str());
    if (rc == 0) {
    ...
    int rc = system(("ros2 run nav2_map_server map_saver_cli --ros-args -p map_subscribe_transient_local:=true -r map:=map_slam" + set_namespace).c_str());
    if (rc == 0) {
      response->result = response->RESULT_SUCCESS;
    } else {
      response->result = response->RESULT_UNDEFINED_FAILURE;
